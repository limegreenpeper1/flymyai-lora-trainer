...
from image_datasets.dataset_mps_bf16 import loader
...
def main():
    ...
    # MPS互換性のためのdtype調整
    if device.type == "mps":
        weight_dtype = torch.bfloat16  # MPSではfloat16が不安定な場合があるため
    ...
    if device.type == "mps":
        # MPSでは量子化前にCPUに配置
        flux_transformer = flux_transformer.cpu()
        text_encoding_pipeline.text_encoder = text_encoding_pipeline.text_encoder.cpu()
        
        # 量子化実行
        quantize(flux_transformer, weights=qint8)
        freeze(flux_transformer)
        quantize(text_encoding_pipeline.text_encoder, weights=qint8)
        freeze(text_encoding_pipeline.text_encoder)
        
        # デバイスに移動
        flux_transformer = flux_transformer.to(device)
        text_encoding_pipeline.text_encoder = text_encoding_pipeline.text_encoder.to(device)
    else:
        # CUDAやCPUの場合
        quantize(flux_transformer, weights=qint8)
        freeze(flux_transformer)
